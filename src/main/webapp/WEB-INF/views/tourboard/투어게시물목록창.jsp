...
<%@page import="java.util.List"%>
 ...
 List<TourBoard> list = (List<TourBoard>) request.getAttribute("tourboards");
...
<div class="container" style="width: 10%;float: left; margin-left:3%; margin-bottom:1%;">
<input type="hidden" name="type" >
<form action="/tourtitlesearch" style="width: 400px;">
<table>
<tr>
<td>
...
<%
     for(int i=0; i<list.size(); i++){
    	 TourBoard tourboards = list.get(i); 
   %>

<tr>
 	<td><input type="hidden" id="no" name="no" value="<%=tourboards.getNo()%>"> <%=tourboards.getNo()%></td>  
	<td><a href="/tourboards/tourboard/<%=tourboards.getNo()%>"> <%=tourboards.getTitle()%> (<%=tourboards.getCount() %>) </a></td>
	<td><%=tourboards.getWriter().getName() %> </td>
	<td><%=tourboards.getRdate()%></td>
	<td><%=tourboards.getViews() %></td>
<%-- 	<td><%if(masterid.equals("admin")){ %><button type="button" class="btn btn-outline-info" onclick="location.href='/qna?no=<%=tourboards.getNo()%>'" >답변하기 </button><%} %> </td> --%>
 </tr>
<% } %>
</table>
...
<div class="row">
 ...
 <!-- 이전 -->
  <%if(블럭시작페이지번호>1){ %>
   <a href="tourboards/<%=블럭시작페이지번호-1%>"> << </a>
    <% } %>
   ...
   <%
    ...
    <a href="tourboards/<%=블럭끝페이지번호+1%>"> >> </a>
    <% } %> 
   </ul> 
</div>

 <div class="col-3">
 <button id="btn" class="btn btn-outline-info" onclick="location.href='/tourboard'" style="margin-left:50%;" >작성</button>
 </div> 
